...
async function getClientList() {
    try {
        const objWithEncriptedCols = await sql.bdConnection('select', `SELECT
            CONCAT(p.idCasos, '|freeText') AS 'ID personas',
            CONCAT(p.nombre, '|freeText') AS 'Nombre',
            CONCAT(p.especificidades, '|freeText') AS 'Especialidad',
            ...
            CONCAT('|freeButton|openModalContactosClienteForm btn btn-block fa-solid fa-user-group|Contactos|', JSON_OBJECT('toggle', 'tooltip', 'placement', 'top', 'btnTblGhfOpenModalEditarCliente', p.idPersonas)) AS 'Contactos',
            CONCAT('|freeButton|openModalEliminarCliente btn btn-block fa-solid fa-trash|Eliminar|', JSON_OBJECT('toggle', 'tooltip', 'placement', 'top', 'btnTblGhfOpenModalEditarCliente', p.idPersonas)) AS 'Eliminar'
        FROM
            Tbl_A_casos AS p;`);
